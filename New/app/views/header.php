...
<body class="<?= htmlspecialchars($bodyClass) ?>">
    <nav class="fixed top-0 left-0 right-0 z-50">
        <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 h-20 sm:h-24 flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-6 lg:gap-12 text-[11px] uppercase tracking-[0.2em] font-bold text-white/70">
                <?php 
                    $parents = [];
                    $children = [];
                    ...
                    try {
                        ...
                        foreach ($parents as $item) {
                            ...
                            if (empty($childItems)) {
                                echo '<a class="hover:text-white transition-colors" href="' . htmlspecialchars($url) . '"' . $target . '>' . htmlspecialchars($item['label']) . '</a>';
                            } else {
                                ...
                                foreach ($childItems as $child) {
                                    ...
                                    echo '<a class="block px-4 py-2 text-[10px] uppercase tracking-[0.2em] text-white/70 hover:text-white hover:bg-white/5 transition-colors" href="' . htmlspecialchars($childUrl) . '"' . $childTarget . '>' . htmlspecialchars($child['label']) . '</a>';
                                }
                                echo '</div></div></div>';
                            }
                        }
        ...
        <div id="mobileMenu" class="md:hidden hidden border-t border-white/10 bg-[#000000] mix-blend-normal opacity-100 backdrop-blur-none">
            <div class="max-w-[1440px] mx-auto px-6 py-6 flex flex-col gap-4 text-[11px] uppercase tracking-[0.2em] font-bold text-white">
                <?php 
                    try {
                        if (empty($parents)) {
                        ...
                        foreach ($parents as $item) {
                            ...
                            if ($hasChildren) {
                                ...
                                echo '</button>';
                                echo '<div id="' . $parentId . '" class="pl-4 flex flex-col gap-3 border-l border-white/10 hidden">';
                                foreach ($childItems as $child) {
                                    [$childUrl, $childExternal] = $buildUrl($child['url']);
                                    $childTarget = $childExternal ? ' target="_blank"' : '';
                                    echo '<a class="text-[10px] text-white hover:text-white transition-colors" href="' . htmlspecialchars($childUrl) . '"' . $childTarget . '>' . htmlspecialchars($child['label']) . '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            } else {
                ...
                ?>
                <a class="px-4 py-2 border border-white/20 hover:border-white transition-colors inline-flex w-fit" href="<?= htmlspecialchars($navCtaHref) ?>"<?= $navCtaTarget ?>><?= htmlspecialchars($navCtaLabel) ?></a>
            </div>
        </div>
    </nav>
    <script>
